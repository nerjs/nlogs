...
class NLoggerCore {
    ...
    formatStr(...args) {
        return this.config.formatters.string(...args)
    }

    formatTime(...args) {
        return this.config.formatters.time(...args)
    }

    groupedEveryTime() {
        if (!this.config.config.groupedEveryTime) return
        const curTime = moment()
        const { formatters, time } = this.config.config
        let momentMethod = ''
        ...
        if (NLoggerCore.lastLog[momentMethod]() !== curTime[momentMethod]()) {
            console.log(formatters.string(formatters.time(curTime), time))
        }

        NLoggerCore.lastLog = curTime
    }
