...
class ConsoleTransport extends Transport {
    ...
    log(msg) {
        ...
        switch (msg.label) {
            ...
            default:
                ...
                if (msg.hasDetails) msgs.push(msg.details)
                console[method](...prefix.flat(), ...msgs)
                break
        }
    }
     ...
     */
    getPrefix(msg) {
        return [this.getTimePrefix(msg), this.getPathPrefix(msg), this.getLabelPrefix(msg)]
    }
    ...
    getPathPrefix(msg) {
        ...
        if (showFile) {
            if (msg.context.filename) {
                const fileArr = msg.context.filename
                    .split('/')
                    .map(s => `${s}`.trim())
                    .filter(s => !!s)
                const lastSegment = fileArr.pop()

                if (fileArr.length) res.push(this.style(`${fileArr.join('/')}`, 'segment'))
                res.push(this.style(lastSegment, 'lastSegment'))
            }
        } else {
            res.push(
        ...
        return res.join(this.style('/', 'segment'))
    }
